...
public class Day20Challenge02Original {
    ...
    private int TILE_MATRIX_MAX_SIZE = 12;
    ...
    public void calculate() throws IOException {
        InputStream reader = getClass().getResourceAsStream("/2020/day20/input.txt");
    ...
    private boolean testDragonSuccess(char[][] imageToShow) {
        ...
        for (int i = 0; i < imageToShow.length - profondeur; i++) {
            for (int j = 0; j < imageToShow[0].length - lengthDragon; j++) {
                if (isFoundDragon(i, j, imageToShow)) {
                    counterDragon++;
                }
        ...
        if (0 < counterDragon) {
            ...
            System.out.println("Dragons: " + counterDragon);
    ...
    private boolean isFoundDragon(int i, int j, char[][] imageToShow) {
        ...
        for (int jj = 0; jj < DRAGON_1.length(); jj++) {
            if (DRAGON_1.charAt(jj) != HASHTAG || (DRAGON_1.charAt(jj) == HASHTAG && HASHTAG == imageToShow[i][j + jj])) {
        ...
        for (int jj = 0; jj < DRAGON_2.length(); jj++) {
            if (DRAGON_2.charAt(jj) != HASHTAG || (DRAGON_2.charAt(jj) == HASHTAG && HASHTAG == imageToShow[i + 1][j + jj])) {
        ...
        for (int jj = 0; jj < DRAGON_3.length(); jj++) {
            if (DRAGON_3.charAt(jj) != HASHTAG || (DRAGON_3.charAt(jj) == HASHTAG && HASHTAG == imageToShow[i + 2][j + jj])) {
