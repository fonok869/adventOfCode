...
public class Day18Challenge01 {
    ...
    public void calculate() throws IOException {
        ...
        try (BufferedReader file = new BufferedReader(new InputStreamReader(reader));) {
            String line;
            Line lineActual = null;
            while ((line = file.readLine()) != null) {
                if (line.length() != 0) {
                    if (lineActual == null) {
                        lineActual = new Line(line);
                    } else {
                        String lineAct = lineActual.getLine();
                        lineActual = new Line("[" + lineAct + "," + line + "]");
                    }
                }
            }
            System.out.println(lineActual.getLine());
            System.out.println("Magnitude: " + calculMagnitude(lineActual.getLine()));
        }
