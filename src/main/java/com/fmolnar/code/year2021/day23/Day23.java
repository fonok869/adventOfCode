...
public class Day23 {
    ...
    public void calculate() throws IOException {
        ...
        System.out.println("First: " + minimumsForDone.stream().mapToInt(s -> s).min().getAsInt());
        ...
        for (byte bit = 1; bit < 5; bit++) {
            ...
            for (int i = 0; i < MAX; i++) {
                ...
                for (int k = 0; k < 2; k++) {
                    ...
                    for (Step step : steps) {
                        if (step.config[SEVEN + FOUR +  bit - 1] == bit) {
            ...
            bitMap.put((SEVEN + bit - 1), 1 *multiplicateur);
            bitMap.put((SEVEN + FOUR + bit - 1), 0);
    ...
    record Step(byte[]config, int cost) {
        ...
        boolean isDone() {
            if (config[SEVEN] == (byte) 1 && config[SEVEN + FOUR] == (byte) 1) {
                if (config[SEVEN + 1] == (byte) 2 && config[SEVEN + FOUR + 1] == (byte) 2) {
                    if (config[SEVEN + 2] == (byte) 3 && config[SEVEN + FOUR + 2] == (byte) 3) {
                        if (config[SEVEN + 3] == (byte) 4 && config[SEVEN + FOUR + 3] == (byte) 4) {
                            return true;
                        }
                    }
                }
        ...
        List<Step> getAllNewMovementUpStep() {
            ...
            for (byte position = SEVEN; position < MAX; position++) {
                ...
                if (amp == (byte) 0) {
                    ...
                    if ((byte) (position - (SEVEN + FOUR - 1)) == amp) {
                        //DONE
