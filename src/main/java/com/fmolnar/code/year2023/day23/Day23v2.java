...
public class Day23v2 {
    ...
    private static Map<Point, Integer> shortestPathWithInverseDijkstra(Set<Point> forests, Set<Point> slopeJobbra, Set<Point> slopLefele, int maxX, int maxY) {
        ...
        PriorityQueue<Node> pqueue = new PriorityQueue<Node>(nodeComparator);
        ...
        while (alreadyVisited.size() != maradekPont && !pqueue.isEmpty()) {
            ...
            if (alreadyVisited.contains(nodeActualToExamine.point)) {
                if(distances.get(nodeActualToExamine)< nodeActualToExamine.cost()){
    ...
    private static void szomszedokKalkulasa(Node nodeActualToExamine, Set<Point> alreadyVisited, Map<Point, Integer> distances, PriorityQueue<Node> pqueue) {
